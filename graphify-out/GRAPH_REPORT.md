...
## Corpus Check
- Corpus is ~32,613 words - fits in a single context window. You may not need a graph.

## Summary
- 1065 nodes · 2095 edges · 66 communities (51 shown, 15 thin omitted)
- Extraction: 74% EXTRACTED · 26% INFERRED · 0% AMBIGUOUS · INFERRED: 539 edges (avg confidence: 0.78)
- Token cost: 111,105 input · 0 output

## Community Hubs (Navigation)
- Qt Workspace Canvas
- Studio App & Project Persistence
- Solver Hierarchy & Strategies
- Architecture Docs: Workspace ADRs
- Manifesto & Algorithm Catalog
- Skyline Algorithm Core
- Core Architecture ADRs
- MaxRects Core Algorithm
- Backlog & Solver Architecture Docs
...
- Generator Registry & Project Domain
- Board Domain & Free Space
- Placement & Collision
- Domain Package
- Solver Objectives & Assembly Solution
- MaxRects Package & Board Orderings
- Domain Explanation & Placement
...
- Skyline Generator
- Free Space & Horizontal/Vertical Generators
...
- **Persistencia de proyecto — implementación de IDE-0004** — masterplan_doc_004_backlog_ide_0004, docs_studio_project_io, docs_studio_open_save_actions, docs_studio_projectmanager [INFERRED 0.85]
...
### Community 1 - "Studio App & Project Persistence"
Cohesion: 0.05
Nodes (40): Persistencia — project_io.py (project_to_dict/from_dict, save/load_project_to/from_file, .bcstudio.json), EventHandler, main(), Application entry point for BoardComposer Studio., Run BoardComposer Studio., EventBus, Event bus for BoardComposer Studio., Simple synchronous event bus. (+32 more)

### Community 2 - "Solver Hierarchy & Strategies"
Cohesion: 0.06
Nodes (46): ABC, Estrategias de optimización (OptimizationStrategy), BaseSolver (ABC), GeometrySolver (solver de produccion), SequentialSolver, SequentialSolver es el Motor v0.1 original del proyecto; se mantiene como implementacion de referencia, no es codigo muerto, build_demo_project(), main() (+38 more)

### Community 3 - "Architecture Docs: Workspace ADRs"
Cohesion: 0.08
Nodes (30): ADR-003 (Event Bus), ADR-010, docs/architecture.md, BoardPieceItem, BoardWorkspace, CommandManager — pilas de undo/redo, DeletePieceCommand, docs/masterplan/DOC-007-UX-Studio.md (+22 more)
...
### Community 6 - "Core Architecture ADRs"
Cohesion: 0.07
...
### Community 7 - "MaxRects Core Algorithm"
Cohesion: 0.08
...
### Community 8 - "Backlog & Solver Architecture Docs"
Cohesion: 0.05
Nodes (40): Algoritmos (docs/algorithms.md), Arquitectura interna del Solver (docs/solver_architecture.md), Fase 1 — Core (Completada), Backlog del Producto (DOC-004), IDE-0001 Workspace interactivo (Completada, P0), IDE-0002 Comparador de algoritmos (Planificada, P0), IDE-0003 Inspector de piezas (Planificada, P0), IDE-0005 Exportación PDF/SVG (Planificada, P1) (+32 more)
...
Cohesion: 0.09
Nodes (11): Command (Protocol: name, redo(), undo()), Protocol, Command, CommandManager, Command manager for undo/redo., Stores undo and redo stacks., Base command protocol for Studio undo/redo., DeletePieceCommand (+3 more)

### Community 12 - "MaxRects Beam vs Classic Route"
Cohesion: 0.11
Nodes (29): BoardOrdering, maxrects/beam.py no reutiliza beam_search.py; candidato a revisar si hace falta mantener ambas implementaciones, beam_search.py: implementacion generica de beam search, Ruta MaxRects con Beam Search (maxrects_beam, beam_width=4), Ruta clasica MaxRects (sin beam search), maxrects_engine.py: tercera via de comparacion (workbench/tools), Familia MaxRects (modulos maxrects_*), Por que hay tantos ficheros maxrects_*: crecimiento incremental con cada algoritmo anadido (+21 more)

### Community 13 - "Beam Search (generic) & MaxRects Beam Docs"
Cohesion: 0.09
...
### Community 14 - "Rectangle Geometry"
Cohesion: 0.10
Nodes (13): Rectangle (primitiva geométrica), Rectangle, FreeSpace, FreeSpaceManager, test_free_space_manager_finds_space(), test_free_space_manager_places_rectangle(), test_free_space_manager_rejects_rectangle_that_does_not_fit(), test_free_space_fits_rectangle() (+5 more)

### Community 15 - "Architecture Docs: Core Layers & CLI"
Cohesion: 0.08
...
### Community 16 - "Generator Registry & Project Domain"
Cohesion: 0.15
Nodes (19): Generadores de una sola solucion vs varias: sin adaptador generico, Project, horizontal_generator(), maxrects_generator(), skyline_generator(), vertical_generator(), generate_horizontal_permutations(), generate_horizontal_solution() (+11 more)

### Community 17 - "Board Domain & Free Space"
Cohesion: 0.11
Nodes (16): Board, place_board_in_first_space(), largest_area_first(), longest_edge_first(), original_order(), test_largest_area_first(), test_longest_edge_first(), test_original_order() (+8 more)
...
### Community 20 - "Solver Objectives & Assembly Solution"
Cohesion: 0.20
Nodes (12): Métricas de puntuación (objectives.py), AssemblySolution, compactness(), material_utilization(), placed_board_ratio(), rotation_ratio(), test_solution_explanation_accepts_values(), test_solution_explanation_defaults() (+4 more)

### Community 21 - "MaxRects Package & Board Orderings"
Cohesion: 0.21
Nodes (13): Paquete solver/maxrects/ (logica geometrica), board_ordering.py: ordenes de tablas compartidos, best_area_fit(), best_bottom_left_fit(), best_long_side_fit(), best_short_side_fit(), test_best_area_fit_prefers_less_waste(), test_best_area_fit_returns_none_without_candidates() (+5 more)

### Community 22 - "Domain Explanation & Placement"
Cohesion: 0.16
Nodes (7): SolutionExplanation (dataclass), SolutionExplanation, SolutionScore, test_solution_score_rejects_negative_values(), test_solution_score_total(), test_solution_dimensions_and_area(), test_solution_with_empty_placements()

### Community 23 - "MaxRects Contact Heuristics"
Cohesion: 0.29
...
Cohesion: 0.19
Nodes (7): Pipeline de candidatos (CandidatePipeline), deduplicate_solutions(), solution_signature(), evaluate(), test_deduplicate_solutions_removes_duplicates(), test_evaluation_returns_score(), test_custom_scoring_weights()

### Community 25 - "MaxRects Core Algorithm (find/place)"
Cohesion: 0.24
...
### Community 28 - "Skyline Generator"
Cohesion: 0.24
...
### Community 29 - "Free Space & Horizontal/Vertical Generators"
Cohesion: 0.22
...
Cohesion: 0.29
Nodes (8): LayoutGenerator, generators_by_name(), _placed(), test_maxrects_places_at_least_as_many_boards_as_skyline(), test_generators_by_name(), test_maxrects_beam_generator_is_registered(), test_maxrects_generator_is_registered(), test_skyline_generator_is_registered()

### Community 31 - "Sequential Solver (reference implementation)"
Cohesion: 0.36
...
### Community 32 - "Layout Bounds"
Cohesion: 0.29
Nodes (3): bounding_rectangle(), test_bounding_rectangle(), test_bounding_rectangle_empty()

### Community 33 - "Candidate Pipeline & Scoring Docs"
Cohesion: 0.29
Nodes (7): Capa solver/, AssemblySolution (dataclass frozen), BoardPlacement (dataclass), SolutionScore (dataclass), Cálculo evaluate(), Reglas de explicación textual, ScoringWeights (scoring_weights.py)

### Community 34 - "Skyline Runner"
Cohesion: 0.43
...
### Community 37 - "Constraints Validator"
Cohesion: 0.50
Nodes (3): respects_constraints(), test_respects_constraints_accepts_valid_solution(), test_respects_constraints_rejects_excess_length()

### Community 38 - "Packing Benchmarks Tests"
Cohesion: 0.83
...
## Knowledge Gaps
- **131 isolated node(s):** `boardcomposer`, `Ruff format check step`, `Ruff lint step`, `Run tests step (pytest)`, `Principle: the program proposes multiple solutions` (+126 more)
...
- **What is the exact relationship between `DT-0002 Arquitectura interna del Solver (Resuelto)` and `Arquitectura interna del Solver (docs/solver_architecture.md)`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **Why does `LayoutService` connect `Studio App & Project Persistence` to `Solver Hierarchy & Strategies`, `Architecture Docs: Core Layers & CLI`?**
  _High betweenness centrality (0.268) - this node is a cross-community bridge._
- **Why does `Board` connect `Board Domain & Free Space` to `Studio App & Project Persistence`, `Solver Hierarchy & Strategies`, `MaxRects Engine`, `Skyline Runner`, `Packing Benchmarks Tests`, `MaxRects Beam vs Classic Route`, `Beam Search (generic) & MaxRects Beam Docs`, `Architecture Docs: Core Layers & CLI`, `Generator Registry & Project Domain`, `Domain Package`, `Skyline Generator`, `Free Space & Horizontal/Vertical Generators`, `Generator Registry Tests`, `Sequential Solver (reference implementation)`?**
  _High betweenness centrality (0.143) - this node is a cross-community bridge._
- **Why does `Project` connect `Generator Registry & Project Domain` to `Studio App & Project Persistence`, `Solver Hierarchy & Strategies`, `MaxRects Engine`, `Skyline Runner`, `Packing Benchmarks Tests`, `MaxRects Beam vs Classic Route`, `Beam Search (generic) & MaxRects Beam Docs`, `Architecture Docs: Core Layers & CLI`, `Board Domain & Free Space`, `Domain Package`, `Skyline Generator`, `Free Space & Horizontal/Vertical Generators`, `Generator Registry Tests`, `Sequential Solver (reference implementation)`?**
