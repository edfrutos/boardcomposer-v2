...
python_path = /Volumes/BACKUPS_PROYECTOS/__03.-Github_Repositories/boardcomposer_v2/.venv/bin/python
...
# --macos-app-name sets CFBundleName, the display name macOS shows in the
# menu bar (top-left, bold) and Dock — a *different* plist key from the
# bundle identifier above. Without it nuitka falls back to a generic "app"
# there too (spotted in v0.3.7, menu bar read "app" instead of the actual
...
extra_args = --quiet --noinclude-qt-translations --macos-signed-app-name=com.efjdefrutos.boardcomposer.studio --macos-app-name="BoardComposer Studio" --macos-app-version=0.3.31 --include-package=google.genai --include-package=openai --include-package=certifi --include-package-data=certifi
