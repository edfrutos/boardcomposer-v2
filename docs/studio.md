...
Una entrada nueva en el menú "Proyecto": "Añadir retal al inventario…" (`AddScrapDialog`, no exige proyecto abierto). El retal se marca consumido **en cuanto se usa** para un tablero, fuera del historial de undo/redo del proyecto a propósito: deshacer el `AddBoardCommand` quita el tablero del proyecto, pero no devuelve el retal al inventario.

### Elegir un retal al añadir tablero — `IDE-0044`

Pedido por el usuario el 17/08/2026, a partir de la ampliación que `DOC-999-Ideas.md` (Candidata 1 de aprovechamiento de retales) había dejado sin acotar: antes había dos entradas de menú separadas ("Añadir tablero…" y "Usar retal del inventario…") — el usuario tenía que acordarse de que existía la segunda para no perderse un retal aprovechable. Ahora es una sola: "Añadir tablero…" (`MainWindow._add_board()`) comprueba primero `self.inventory.list_available()` — si hay retales, abre `UseScrapDialog` (todos los disponibles, sin filtrar por material, para no ocultar uno válido antes de que el usuario diga qué tablero quiere) con un botón nuevo, "Tablero nuevo…", que cierra el diálogo (`reject()`) marcando `dialog.new_board_requested = True`. `_add_board()` distingue tres salidas: aceptado (`_apply_scrap_as_board()`, misma lógica de siempre — `StudioBoard` + `AddBoardCommand` deshacible + `consume()` fuera del undo), rechazado con `new_board_requested` (cae a `_add_new_board()`, el `BoardDialog` de siempre) o cancelado sin más (no hace nada). Con el inventario vacío, `_add_board()` va directo a `_add_new_board()` — ningún cambio de comportamiento para quien no usa retales.

## Biblioteca de materiales — `studio/project/materials_library.py`, `studio/materials_service.py`

`IDE-0041`, primer bloque construido de la Fase 5 (Ecosistema) del Roadmap tras `DEC-0020`. Catálogo reutilizable y exportable de materiales (nombre, grosor, proveedor, precio) — reemplaza escribir el material a mano en cada tablero/pieza por un desplegable, y permite compartir el catálogo entre instalaciones de Studio como un fichero CSV.

`materials_library.py` es la capa pura (sin Qt, testeada en `tests/test_materials_library.py`) — mismo patrón SQLite que `scrap_inventory.py`, pero es dato de referencia sin ciclo de vida: `add_material()`/`update_material()`/`delete_material()`/`list_materials()` (ordenado por nombre y luego grosor). A diferencia de un retal, eliminar un material es un `DELETE` real, no un flag de "consumido" — no hay historial que preservar. `add_materials_bulk()` inserta una lista completa en una única transacción SQLite (usada por la importación CSV): cualquier fila inválida o id repetido deshace la transacción entera, sin dejar el catálogo a medio importar.

`materials_csv.py` (testeado en `tests/test_materials_csv.py`) exporta/importa el catálogo como CSV — columnas `id,name,thickness_mm,provider,price,price_unit`. Mismo patrón de `_column_from_row()` con alias por columna que `csv_import.py`, aplicando la lección de `DT-0026` de forma proactiva (no como reacción a un bug ya reportado): acepta `name`/`nombre`, `thickness_mm`/`grosor_mm`/`grosor`, etc., sin distinguir mayúsculas. `price_unit` acepta `board`/`tablero` y `m2`/`m²` en la importación; la exportación siempre escribe los códigos internos (`board`/`m2`).

`materials_service.py` (`MaterialsLibraryService`) es el envoltorio con Qt: mismo mecanismo `QStandardPaths.AppDataLocation` que `ScrapInventoryService`, fichero propio (`materiales.db`, no `retales.db` — son dos recursos de taller independientes). `MainWindow` recibe una instancia por parámetro (`materials=None` construye la real), deliberadamente fuera de `StudioServices` por el mismo motivo que el inventario de retales: es un recurso del taller, no estado de un proyecto.

Una entrada nueva en el menú "Proyecto": "Biblioteca de materiales…" abre `MaterialsLibraryDialog` (`studio/dialogs/materials_dialogs.py`), que a diferencia del resto de diálogos del proyecto no es puro — posee el servicio directamente y persiste cada alta/edición/baja/importación al momento, mismo patrón que un gestor de contactos o marcadores, porque el catálogo no participa del undo/redo de ningún proyecto. Incluye importar/exportar CSV (`QFileDialog`). `BoardDialog`/`PieceDialog` reciben un parámetro `materials` opcional (lista de nombres, sin duplicados) que convierte el campo "Material" de `QLineEdit` a un `QComboBox` editable — sigue aceptando texto libre para un material que no está en el catálogo, el catálogo puede empezar vacío sin bloquear nada.
...
## Buscar actualizaciones — `studio/update_check.py`, `studio/update_installer.py`, `studio/self_update.py`

`IDE-0032` solo comparaba versiones y mostraba un enlace a la página de la release — nunca descargó ni instaló nada, ni siquiera tras `IDE-0043`. La cadena completa hoy:

- **`update_check.py`** (`check_for_update()`) compara `STUDIO_VERSION` contra un Gist público (`version.json`: `version`/`release_url`/`dmg_url`) — no la API de Releases de GitHub, porque el repo es privado y una petición sin autenticación devuelve `404` (`DT-0025`). `package-studio.yml` actualiza el Gist solo en cada release (`DT-0028`).
- **`update_installer.py`** (`download_dmg()`) descarga el `.dmg` a `~/Downloads` con progreso y cancelación cooperativa. El `.dmg` en sí vive en un segundo repo, público y sin código, `edfrutos/boardcomposer-releases` (`IDE-0043`, `DEC-0021`) — los assets de una release de un repo privado también exigen autenticación.
- **`self_update.py`** (`IDE-0045`, `DEC-0023`) sustituye el `.app` en marcha por el descargado y lo relanza, para que no haga falta arrastrar nada a mano. `running_app_bundle_path()` detecta si Studio corre desde un `.app` empaquetado (vía `sys.executable`) — fuera de uno (fuente, tests) devuelve `None`, y el flujo cae intacto al de `IDE-0043` (solo abrir el `.dmg`). `install_update()` monta la imagen (`hdiutil`), copia el `.app` con `ditto` (preserva los atributos extendidos de los que depende la firma — `cp -R`/`shutil.copytree` no), comprueba `CFBundleShortVersionString` contra la versión esperada, y sustituye con dos renames atómicos dentro del mismo directorio: nunca hay un instante en que la ruta no tenga ningún `.app` válido, aunque no se conserve ningún `.bak` tras completar (elección explícita del usuario). No hace falta ninguna clave de firma de actualizaciones nueva — el `.dmg` que instala ya está firmado y notarizado por el CI. `relaunch(bundle, wait_for_pid=os.getpid())` abre el `.app` nuevo como proceso independiente (`start_new_session=True`) — pero no de inmediato: llamar a `open` para la misma ruta mientras el proceso viejo sigue vivo puede hacer que Launch Services de macOS trate la petición como "esta app ya está en marcha" y no arranque nada nuevo (`DT-0029`, confirmado en real: la sustitución de ficheros se completó, pero la app nunca se reabrió). En su lugar, `relaunch()` lanza un `/bin/sh -c` desacoplado que sondea `kill -0 <pid>` hasta que el proceso indicado desaparece de verdad, y solo entonces ejecuta `open -n` (fuerza una instancia nueva, por si Launch Services sigue confundido sobre un bundle identifier recién liberado). En macOS/APFS, sustituir los ficheros de un ejecutable mientras corre es seguro — sigue usando el inodo antiguo hasta que sale — así que la sustitución del `.app` (más arriba) no tiene este problema; solo el relanzamiento lo tenía.
- **`MainWindow._offer_to_download_update()`** orquesta: pide confirmación (el texto avisa de que la app se cerrará, si `running_app_bundle_path()` no es `None`), descarga con `QProgressDialog`, y solo si hay un `.app` que sustituir comprueba que se puede cerrar con seguridad (`_maybe_save_and_confirm_close()`, extraída de `closeEvent()` — misma comprobación de cambios sin guardar, reutilizada aquí para no arriesgar un proyecto abierto) **antes** de tocar cualquier fichero. Un fallo de `install_update()` no deja el proceso a medias: avisa y cae de vuelta al `.dmg` ya descargado, abierto a mano.

**Fuera de alcance:** Sparkle (el framework) — pensado para bundles de Xcode, empotrarlo en un `.app` de Nuitka exigiría firmar componentes de terceros dentro del bundle, la misma fragilidad que ya dio tres bugs reales la primera vez que se firmó contra un certificado real (`DT-0011`). Elevación de privilegios para instalar en una ruta protegida — si no hay permiso de escritura, avisa y cae al `.dmg` manual, no pide `sudo`. Windows/Linux (Nuitka solo empaqueta macOS/arm64, `DT-0011`).
