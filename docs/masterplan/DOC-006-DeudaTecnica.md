...
| DT-0013 | DT-UX | Studio no permite crear ni editar tablas o piezas desde la interfaz: el menú "Editar" solo tiene deshacer/rehacer/rotar/eliminar (todas sobre piezas ya existentes), el Inspector (`inspector_panel.py`) es de solo lectura, y `BoardWorkspace._add_board()`/`_add_pieces()` únicamente dibujan datos que ya están en el `StudioProject` cargado — no crean nada nuevo. Hoy la única forma de meter datos reales es escribir a mano un `.bcstudio.json` y abrirlo. Detectado en la misma sesión que `DT-0012`. `docs/masterplan/ui/flows/FLW-006-Editar-Proyecto.md` ya documenta el flujo previsto (validación, historial, deshacer/rehacer) pero nunca se implementó. Pendiente de acotar alcance (diálogos de alta, validación, integración con `CommandManager`) antes de programarlo — no resuelto en esta sesión, decisión explícita de tratarlo como una fase aparte. | 🔴 Abierto |
