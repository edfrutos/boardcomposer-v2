...
**Última revisión:** 16/07/2026 (segunda revisión del día)
...
| DT-0014 | DT-UX | `BoardDialog`/`PieceDialog` (`studio/dialogs/`) no piden grosor (`thickness_mm`) ni cantidad al crear un tablero o pieza — solo Id/Largo/Ancho/Material. El grosor ni siquiera existe en `StudioBoard`/`StudioPiece` (a diferencia del `Board` del Core, que sí lo tiene); la cantidad tampoco tiene ningún campo — cada pieza es una entrada individual con id único, así que "cantidad" implicaría o bien un campo numérico nuevo, o duplicar automáticamente N piezas con ids derivados (`p1`, `p1-2`…). Detectado probando datos reales tras `DT-0013`. Pendiente de acotar alcance (¿grosor por pieza, por tablero, o ambos? ¿cantidad = campo o duplicado automático?) antes de programarlo — mismo criterio que `DT-0013`: se trata como fase aparte, no resuelto en esta sesión. | 🔴 Abierto |
| DT-0015 | DT-UX | El nodo "Soluciones" del Explorer (`studio/main_window.py`, `_reload_explorer()`) se crea y se añade al árbol pero nunca se rellena — permanece vacío siempre, tanto si se ha calculado/comparado un layout como si no. Da la impresión de una funcionalidad (listado de soluciones generadas) que no existe. Detectado probando datos reales tras `DT-0013`. Pendiente de acotar qué debe mostrar (¿la última solución calculada? ¿historial de comparaciones de la sesión?) y si al seleccionar una entrada debe hacer algo (aplicarla, previsualizarla) antes de programarlo. | 🔴 Abierto |
