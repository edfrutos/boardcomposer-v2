...
| DEC-0022 | De la priorización de retales que `DEC-0019` había dejado sin acotar (Candidata 1, `DOC-999-Ideas.md`), el usuario descartó el 17/08/2026 que el solver la decida solo — prefiere que siga siendo una elección suya, pero **surgida en el momento justo** en vez de tener que acordarse de un menú aparte ("Usar retal del inventario…") que no tenía ninguna relación visible con "Añadir tablero…". Acotado en la misma sesión: al pulsar "Añadir tablero…", si el inventario tiene algo disponible, se ofrecen **todos** los retales (sin filtrar por material — filtrar antes de que el usuario diga qué tablero quiere podría ocultar uno válido) como alternativa a un tablero nuevo, en un único diálogo (no un selector previo y luego otro diálogo). Con el inventario vacío, el flujo no cambia nada respecto a hoy. Construido como `IDE-0044` | ✅ Vigente |
| DEC-0023 | El usuario retomó el 18/08/2026 la candidata de mayor alcance de `DOC-999-Ideas.md` ("Auto-actualización de BoardComposer Studio") tras sentir en real la fricción de `IDE-0043`: cerrar la app y arrastrar el icono a mano. Se descarta integrar Sparkle de verdad (el framework) — pensado para bundles de Xcode, empotrarlo en un `.app` de Nuitka exigiría firmar componentes de terceros dentro del bundle, mismo tipo de fragilidad que ya dio tres bugs reales la primera vez que se firmó contra un certificado real (`DT-0011`); tampoco hace falta una clave de firma de actualizaciones nueva, el `.dmg` que instala ya está firmado y notarizado por el CI. Se construye en su lugar un actualizador propio: sustituye el `.app` en su sitio (rename atómico dentro del mismo directorio, sin backup persistente tras completar — elección explícita del usuario, "copia directa, sin respaldo") y relanza, todo antes de cerrar el proceso en marcha — en macOS/APFS, sustituir los ficheros de un ejecutable mientras corre es seguro, el proceso sigue usando el inodo antiguo hasta que termina. Instala siempre en la ruta desde la que se ejecuta hoy (elección del usuario), nunca fuerza `/Applications`. Solo se activa si hay un proyecto sin cambios sin guardar — reutiliza la misma comprobación de `closeEvent()` (`_maybe_save_and_confirm_close()`) antes de tocar ningún fichero, para no arriesgar el trabajo del usuario. Construido como `IDE-0045` | ✅ Vigente |
| DEC-0024 | El límite de gasto de GitHub Actions se agotó el 20/08/2026 (runners de macOS, ~10x un minuto de Linux, en un repo privado) — el tag `v0.3.29` ni llegó a arrancar. Tres salidas: hacer el repo público (Actions gratis e ilimitadas, pero expone `billing.py`/`stripe_billing.py`, justo lo que `DT-0025` decidió no hacer — revertiría esa decisión), subir el límite de gasto (sigue costando, no arregla nada de fondo), o un runner autoalojado en el propio Mac del usuario. El usuario eligió la tercera — gratis, repo sigue privado, usa hardware que de todas formas está encendido para instalar cada build a mano. `package-studio.yml` pasa de `runs-on: macos-latest` a `runs-on: [self-hosted, macOS]`; `ci.yml` (Linux, ya barato) no se toca. Efecto colateral real, corregido en el mismo cambio: el paso de firma cambiaba el keychain **por defecto** del sistema sin restaurarlo nunca — inofensivo en una VM efímera de GitHub, pero en un Mac que se reutiliza release tras release iría dejando el keychain por defecto apuntando a una ruta de build y acumulando ficheros sueltos. El keychain temporal pasa a tener nombre único por ejecución y un paso nuevo (`if: always()`) restaura el original y borra el temporal, incluso si el job falla a mitad. | ✅ Vigente |
