...
- **`export/`** — exportación del estado actual del workspace a fichero: `svg_export.py` reutiliza `boardcomposer.export.solution_to_svg()` del Core; `pdf_export.py` dibuja lo mismo vía `QPainter`/`QPdfWriter` (Qt, por eso vive en Studio y no en el Core); `dxf_export.py` (`IDE-0022`) reutiliza igual `boardcomposer.export.solution_to_dxf()`, ya existente desde `IDE-0012` pero sin conectar a Studio hasta ahora. `json_export.py` (`IDE-0022`) es un formato propio, no reutiliza `presenters.solutions_to_json()` del CLI/API — ese está pensado para varias candidatas con una `OptimizationStrategy` detrás, y el layout actual de Studio no tiene ninguna una vez el usuario mueve piezas a mano. `solution_bridge.py` convierte `StudioProject` (piezas + colocaciones) a un `AssemblySolution` del Core para que los cuatro exportadores reutilicen la misma geometría.
