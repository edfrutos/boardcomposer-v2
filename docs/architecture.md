...
- **`io/`** — `load_project_from_csv()`, la única fuente de importación integrada hoy (RF-002 solo cubre CSV; Excel sigue pendiente). `importer_by_name()` resuelve tanto `"csv"` como los importadores registrados por plugins (`IDE-0008` Fase D, ver `plugins/` más abajo).
- **`export/`** — `solution_to_svg()`, único exportador integrado hoy. `exporter_by_name()` resuelve tanto `"svg"` como los exportadores registrados por plugins (`IDE-0008` Fase D).
