...
No cubre `boardcomposer-studio-remote` (no expone su versión por HTTP — el script imprime un recordatorio explícito de reconstruirlo desde el mismo tag y confirmarlo con "Ayuda → Acerca de") ni automatiza la reconstrucción. Convierte el desfase de invisible en detectable, no lo impide — automatizarlo del todo (cron en el runner autoalojado, o CI/CD de despliegue) queda como decisión aparte. Primer uso real pendiente: reconstruir el contenedor de la API para que sirva el campo (el `v0.3.42` desplegado es anterior) y correr `check-deploy` con las credenciales.
