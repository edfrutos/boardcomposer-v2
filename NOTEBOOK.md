...
## 2026-08-23 - Rebuild real de la API en la VPS: `v0.3.9` → `v0.3.36`, `DT-0034`

Al revisar el estado de la web app se confirmó lo que `CHECKLIST-operativa.md` ya marcaba como "puede estar desfasado": la API de `bc.efjdefrutos.com` corría `v0.3.9` (`pip show boardcomposer`) — **27 versiones** por detrás del repo, sin ningún rebuild desde el 31/07/2026 (`043caf1`). El usuario ejecutó `git pull` + `docker build` + `docker stop/rm` + `docker run` por SSH; el contenedor nuevo entró en bucle de reinicio (`502 Bad Gateway`).

Causa (`DT-0034`): `sqlite3.OperationalError: unable to open database file` en `billing.init_db()` — el `Dockerfile` solo hace `chown` de `/app` para `appuser` (UID 1000, sin privilegios), nunca del punto de montaje `/data` del volumen `boardcomposer-data`, que Docker crea con propietario `root:root` cuando el volumen es nuevo o se recreó. Segunda vez que pasa exactamente esto — ya se había corregido a mano el 31/07/2026 (`IDE-0020`, ver `MASTERPLAN.md`), pero el arreglo nunca llegó a `docs/deploy.md`, así que no había ningún guardarraíl la siguiente vez que alguien reconstruyó el contenedor semanas después. Resuelto con un contenedor auxiliar como root (`docker run --rm -v boardcomposer-data:/data alpine chown -R 1000:1000 /data`) + `docker restart` — confirmado con `pip show` (`0.3.36`), `/health` (`200`) y `/strategies` con clave (`200`) contra la VPS real. `docs/deploy.md` gana el paso de `chown` explícito en el procedimiento de actualización, para que no dependa de la memoria de nadie una tercera vez.

Lección repetida: un arreglo aplicado a mano en producción, sin escribirlo en el procedimiento documentado, es deuda técnica que vuelve — mismo patrón que ya dejaron registrado `DT-0024`/`DT-0028` con otros pasos manuales (versión de `studio/_version.py`, Gist de actualizaciones) antes de ganar guardarraíles automáticos.

El mismo día, el usuario comparó capturas del menú "Proyecto" entre la app local y `studio.efjdefrutos.com` (Studio por navegador vía noVNC) y detectó que al servidor le faltaban "Añadir retal al inventario…" e "Biblioteca de materiales…" (`IDE-0039`/`IDE-0041`, en la app local desde `v0.3.18`/`v0.3.23`). Mismo diagnóstico que la API unas horas antes: el contenedor de Studio remoto (`Dockerfile.studio`, `docs/deploy-studio-remote.md`) es un despliegue aparte con su propio ciclo de reconstrucción manual, y llevaba igual de desfasado. Reconstruido por SSH siguiendo `docs/deploy-studio-remote.md` (`git pull` — checkout ya compartido con la API, ya en `v0.3.36` — `docker build -f Dockerfile.studio`, `docker stop`/`rm`/`run`); un primer intento falló por un despiste real al copiar el comando (`docker build -f Dockerfile.studio -t boardcomposer-studio-remote` sin el `.` final del contexto de build — "requires exactly 1 argument"), y un segundo intento chocó con el contenedor viejo aún vivo (`Conflict: container name already in use`) antes de pararlo/quitarlo correctamente. A diferencia de `DT-0034`, no hizo falta ningún `chown` — registrado como `DT-0035`. Confirmado visualmente por el usuario: `0.3.36` en "Acerca de", paridad completa en el menú "Proyecto".

Nota de seguridad, resuelta el mismo día: en el proceso de diagnosticar y reconstruir ambos contenedores, el usuario compartió en el chat, en texto plano, `BOARDCOMPOSER_API_KEY`, la contraseña de `auth_basic` de nginx (`bc-studio`), `VNC_PASSWORD` (con el mismo valor que `BOARDCOMPOSER_API_KEY` — credencial reutilizada entre dos servicios) y una clave real de `ANTHROPIC_API_KEY`. Las cuatro rotadas ese mismo 23/08/2026: clave de Anthropic revocada en la consola y sustituida por una nueva, `BOARDCOMPOSER_API_KEY`/`VNC_PASSWORD` regeneradas con `openssl rand` (ya no coinciden entre sí), contraseña de `auth_basic` cambiada con `htpasswd`. Confirmado por el usuario contra los contenedores reales recreados con los valores nuevos.

Con la web ya al día, el usuario subió por `scp` a `/root/studio-compartido/` tres CSV reales del taller (piezas y tableros/retales) para importarlos desde Studio remoto. Antes de subirlos se validaron uno a uno contra las reglas reales de `board_csv_import.py`/`csv_import.py` (no una revisión visual): dos limpios (`BCS_v2_piezas_reales.csv`, `BCS-v2_retales_garaje.csv`); uno roto de raíz (`BCS-v2_Tableros retales.csv` — cabecera con 6 columnas pero filas de datos con solo 5 valores, el nombre del material colándose en la columna `quantity` y `material` quedando vacía en las 20 filas; además una fila con un punto en vez de coma que fusionaba dos dimensiones en un número) — descartado sin arreglar porque resultó tener exactamente los mismos 20 ids que `BCS-v2_retales_garaje.csv`, ya limpio; y uno con datos redundantes (`01.listado_gral_tablas_garaje.csv`, 22 filas, y `BCS_v2_medidas_piezas.csv`, 13 filas, describían por dimensiones el mismo lote de tablas con dos numeraciones de id distintas) — el usuario eligió quedarse con el de 22. Ese fichero tenía además ids inconsistentes (letra "O" en vez de cero desde la fila 3, `TABO3`…`TABO22`) y un material `aglomerado_cant` con pinta de valor cortado a medias — corregidos a `TAB03`…`TAB22` y `aglomerado` antes de subirlo, verificado de nuevo contra el validador tras el cambio.

Importados los tres sin incidencias. Al revisar el resultado en el lienzo, el usuario reportó con captura que la etiqueta de un tablero delgado (`TAB01`, 460×40mm, del CSV recién importado) se veía desproporcionadamente grande, invadiendo el rectángulo — `DT-0036`: `create_board_item()` (`studio/workspace/board_item.py`) usaba una fuente fija de 48pt y un offset vertical fijo (-60), sin relación con el tamaño del tablero; mismo tipo de bug que ya se había corregido una vez para las piezas (elisión por ancho) pero nunca se llevó a los tableros. Resuelto escalando el tamaño de fuente al lado más corto del tablero (12-48pt) y calculando el offset vertical con la métrica real de la fuente en vez de un número fijo — dos tests nuevos lo blindan. 1168 tests en verde. Publicado como `v0.3.37`, sin más cambios de producto.

`v0.3.37` falló al notarizar (`DT-0037`): `codesign` firmó bien, pero `notarytool` rechazó las credenciales con `401`. La contraseña específica de aplicación del Apple ID (`APPLE_APP_PASSWORD`, sin tocar desde el 01/08/2026) se había invalidado, sin ningún aviso previo — no ligada a este uso concreto, sino al estado de seguridad general de la cuenta. Al intentar arreglarlo, el usuario generó una contraseña nueva pero la guardó bajo un nombre de secret distinto (`APPLE_APP_SPECIFIC_PASSWORD`), así que el workflow siguió leyendo la vieja sin que nadie se diera cuenta — mismo patrón de "secret nuevo, nombre equivocado, nadie lo lee" que `DT-0024`/`DT-0028`.

En vez de solo corregir el nombre, se acotó con el usuario un cambio más de fondo: `notarytool` pasa a autenticarse con una clave de App Store Connect (`--key`/`--key-id`/`--issuer`) en vez de Apple ID + contraseña de aplicación — scoped solo a notarización, sin depender del ciclo de vida de seguridad de la cuenta de Apple. `scripts/sign_and_notarize.sh` y `package-studio.yml` actualizados (paso nuevo que decodifica el `.p8` en `$RUNNER_TEMP` y lo borra en la limpieza final). Publicado como `v0.3.38` para ejercitar el pipeline nuevo de principio a fin.

Verificarlo en real llevó cinco vueltas, cada una con un fallo distinto — el pipeline llegaba cada vez más lejos, señal de que sí convergía, mismo patrón que ya se vio con el runner autoalojado (`DT-0030`-`DT-0033`):

1. `--issuer` con formato no-UUID — el usuario había puesto un valor equivocado.
2. El mismo error tras "corregirlo" — un carácter de más (probablemente un salto de línea) colado en el secret al subirlo con `gh secret set --body`. Resuelto subiéndolo con `printf '%s' ... | gh secret set` en vez de `--body`, que no añade nada al final.
3. `invalidPEMDocument` — el `.p8` que llegó a GitHub no coincidía con el fichero local ya verificado. Confirmado el fichero local con `base64 -i archivo | base64 -D | diff - archivo` antes de volver a subirlo.
4. Un último `401` genérico, con las tres piezas ya de buen formato por separado pero sin autenticar juntas. Resuelto probando el trío completo en local (`xcrun notarytool history --key/--key-id/--issuer`) y subiendo los tres secrets **desde las mismas variables de la misma sesión de shell** que se acababan de probar con éxito — para que la prueba y la subida usaran exactamente los mismos valores, sin margen a que se mezclara una versión antigua de alguno de los tres entre reintentos.

`v0.3.38` publicada con éxito: `.app` firmado, notarizado, grapado, `.dmg` en la release de GitHub, en el repo público de releases y Gist actualizado — pipeline completo en 41m31s. `DT-0036` y `DT-0037` cerrados.

## 2026-08-25 - Bug encontrado antes de activar Stripe en real (`DT-0038`)

El usuario pidió activar Stripe en producción — el último pendiente de negocio, con `IDE-0021` ya construido desde el 01/08/2026 pero inactivo. Antes de crear ningún Price real en Stripe, revisando cómo encajarían los números del plan (`básico` 9€/mes + 300 incluidas + 0,05€/extra) contra el código existente, salió un fallo de diseño real, nunca ejercitado porque Stripe nunca había estado configurado de verdad: `report_overage()` solo reporta a Stripe las unidades que ya son overage (nunca las que están dentro de cuota), pero `create_customer_and_subscription()` creaba una única Subscription sobre un solo Price — el modelo "obvio" para expresar "cuota base + overage" en Stripe es un precio graduado con el corte de tramo en la cuota incluida (300/1500). Con ese modelo, el uso acumulado que ve Stripe en un mes es solo la suma de overage reportado, que casi nunca llega a cruzar un corte puesto en 300 o 1500 — el overage real se factura en silencio como 0€, sin ningún error visible (`report_overage()` es *best-effort* a propósito).

Arreglado antes de tocar Stripe de verdad (`DT-0038`): `create_customer_and_subscription()` pasa a crear una Subscription de **dos** ítems — un Price normal recurrente para la cuota fija (sin metering, Stripe lo cobra solo cada periodo) y un Price medido **sin tramos** para el overage (un precio fijo por unidad, sin ningún corte que se pueda desincronizar). Solo se guarda el id del ítem de overage — el de cuota fija no necesita seguimiento, Stripe lo factura por su cuenta. `is_configured()` exige las dos variables de cada plan a la vez (`STRIPE_PRICE_BASICO`/`STRIPE_PRICE_BASICO_OVERAGE`, `STRIPE_PRICE_PRO`/`STRIPE_PRICE_PRO_OVERAGE`), para no crear nunca una suscripción a medias. `docs/deploy.md`/`docs/architecture.md` actualizados. 1170 tests en verde (2 nuevos).

Lección: revisar el diseño de facturación contra el modelo real de datos de Stripe *antes* de crear productos con dinero real, no después — un fallo de este tipo no habría dado ningún error, solo habría cobrado de menos a clientes reales sin que nadie se enterara hasta la reconciliación contable.

Publicada `v0.3.39` (solo el arreglo de `DT-0038`, verificado en real: firma, notarización y `.dmg` en 45m48s). El usuario empezó a crear los 4 Price reales en Stripe: los dos de cuota fija salieron bien a la primera (`Usage Type: licensed`), pero los dos de overage salieron con `Interval`/`Usage Type` vacíos — precios de un solo pago, no recurrentes ni medidos, por no haber marcado "Recurring" + "Usage is metered" al crearlos. Al corregirlo, Stripe pidió un **medidor** ("Se requiere un medidor para la facturación basada en el consumo") antes de dejar crear el precio de overage: la cuenta usa el sistema nuevo de Stripe (**Billing Meters**), que sustituye a la API de `SubscriptionItem.create_usage_record` que usaba el código.

Confirmado contra la documentación oficial de Stripe antes de tocar nada (`DT-0039`) — un Meter Event nuevo se reporta por `stripe_customer_id` + un `event_name` fijo del medidor, no por el ítem de la suscripción. `report_overage()` cambia de firma: recibe `(stripe_customer_id, plan)` en vez del id del ítem, y llama a `stripe.billing.MeterEvent.create()`. `event_name` por plan queda fijo en código (`boardcomposer_basico_overage`/`boardcomposer_pro_overage`) — el usuario debe usar esos literales exactos al crear los dos medidores en el Dashboard. `create_customer_and_subscription()` no cambia de forma; el id del ítem de overage que devuelve/guarda pasa a ser solo informativo. Tests actualizados, 1171 en verde.

Publicada `v0.3.40` con este segundo arreglo — segunda vez en la misma tarde que se encuentra y corrige un problema de facturación antes de mover dinero real, ninguno de los dos reportado por un cliente porque todavía no hay ninguno con Stripe activo.

Con `v0.3.40` en la VPS y los 4 `price_...` reales creados en Stripe (dos de cuota fija bien a la primera; los dos de overage necesitaron rehacerse tras descubrir en directo que la cuenta exige un *Meter* — mismo hallazgo de `DT-0039`, confirmado sobre la marcha con el propio Dashboard en vez de solo con la documentación), la primera alta real de cliente (`manage_keys.py create taller-prueba --plan pro`) reveló un tercer fallo — `DT-0040`: `Subscription.create()` con cobro automático (el modo por defecto de Stripe) exige una tarjeta ya asociada al `Customer`, y uno recién creado por el CLI de admin nunca la tiene. Acotado con el usuario: `manage_keys.py` da de alta clientes con los que ya hay una relación directa, no un checkout de autoservicio — factura por email (`collection_method="send_invoice"`, `days_until_due=15`) encaja mejor que exigir una tarjeta de antemano. 1171 tests en verde. Publicada `v0.3.41` con este tercer arreglo, pendiente de reconfirmar el alta real contra la VPS una vez desplegada.

Tres fallos de facturación seguidos, ninguno reportado por un cliente real — todos encontrados por el propio proceso de activar Stripe con cuidado, probando cada paso contra la cuenta real antes de darlo por bueno, en vez de asumir que "ya estaba construido" (`IDE-0021`, 01/08/2026) significaba "ya funciona".

Desplegada `v0.3.41` en la VPS: el error de "sin método de pago" desapareció, confirmando que `DT-0040` funciona — pero salió un cuarto fallo en la misma prueba, `DT-0041`: `Missing email. In order to create invoices that are sent to the customer, the customer must have a valid email.` Obvio en cuanto se ve el mensaje (facturar por email exige, literalmente, un email) pero invisible hasta que Stripe lo exigió — `create_customer_and_subscription()` nunca pasaba ninguno, y `customer_id` (el identificador que usa `manage_keys.py`, p. ej. `"taller-1"`) no tiene por qué ser una dirección de correo. Resuelto: `email` pasa a ser un parámetro obligatorio de `create_customer_and_subscription()`, y `manage_keys.py create` gana `--email`, exigido con un mensaje de error propio (no un traceback de Stripe) si el plan es de pago y Stripe está configurado. 1171 tests en verde. Publicada `v0.3.42` con este cuarto arreglo, pendiente de reconfirmar el alta real contra la VPS.
...
Retomado el pendiente de "dar un guardarraíl tipo `check_project.py`" para el desfase de los contenedores de la VPS. `DT-0034` (API en `v0.3.9`, 27 releases atrás) y `DT-0035` (Studio remoto igual de viejo) se descubrieron las dos veces solo al comprobarlo a mano — no había nada que lo hiciera saltar.

El obstáculo era que no había forma de leer la versión desplegada sin entrar por SSH: `/health` devolvía solo `{"status": "ok"}`. Añadido `version` a `/health` (`importlib.metadata.version("boardcomposer")` — la que reporta `pip show` dentro del contenedor; funciona ahí porque la API corre como paquete instalado, a diferencia del `.app` de Studio congelado). Es versión de build/despliegue, no de contrato de API — `DOC-008` sigue con el versionado de contrato pendiente, y esto no lo toca.

`scripts/check_deployment.py` (stdlib solo, mismo molde que `check_project.py`): hace `GET <api-url>/health`, compara `version` con la de `pyproject.toml`, y sale con código 1 si no coinciden o si el `/health` no trae el campo (contenedor anterior al guardarraíl), 2 si no puede alcanzarlo. `make check-deploy` lo envuelve; toma `BC_DEPLOY_AUTH="usuario:contraseña"` para el HTTP Basic de nginx (`/health` en sí está exento de `X-API-Key`, solo el `auth_basic` de Plesk va delante). Probado en real contra `bc.efjdefrutos.com` sin credenciales: alcanza la VPS, recibe el `401` del `auth_basic` y sale con código 2 y la pista correcta. 5 tests nuevos (`tests/test_check_deployment.py`, `_fetch_health` mockeado). `docs/deploy.md` gana un paso 9 y `docs/architecture.md` documenta la forma nueva de `/health`.

No cubre `boardcomposer-studio-remote` (no expone su versión por HTTP — el script imprime un recordatorio explícito de reconstruirlo desde el mismo tag y confirmarlo con "Ayuda → Acerca de") ni automatiza la reconstrucción. Convierte el desfase de invisible en detectable, no lo impide — automatizarlo del todo (cron en el runner autoalojado, o CI/CD de despliegue) queda como decisión aparte.

### Despliegue de `v0.3.42` en la VPS (mismo día)

El usuario reconstruyó los dos contenedores por SSH desde el mismo checkout de `main`, con el env volcado a un fichero (`docker exec … env | grep …`) y reinyectado con `--env-file` — para no re-teclear los secretos ni arriesgarse a olvidar una variable (la trampa de "billing silenciosamente desactivado" de `DT-0034`).

- **API:** `curl http://127.0.0.1:5050/health` → `{"status":"ok","version":"0.3.42"}`, y lo mismo desde fuera contra `bc.efjdefrutos.com`. El campo `version` ya sirve para el guardarraíl.
- **Studio remoto:** reconstruido igual; "Ayuda → Acerca de" → `0.3.42`. El primer acceso a `studio.efjdefrutos.com/vnc.html` daba "Failed to connect to server" y volvía a la pantalla de "Conectar". Diagnóstico por capas: websockify sirve `/vnc.html` (200), x11vnc responde `RFB 003.008` en 5900, nginx tiene las cabeceras de websocket, y el log de x11vnc mostraba `accepted_client` + `rfbProcessClientProtocolVersion: client gone` — el ws subía entero y el navegador cortaba en el handshake. Era **caché del navegador** (noVNC viejo): en ventana privada / con recarga forzada conecta bien. La infra de Studio (`Dockerfile.studio`, entrypoint) no había cambiado desde antes de `v0.3.36`, coherente con que no fuera un problema del backend.

Dos incidencias menores por el camino: `python3 scripts/check_deployment.py` en el host de la VPS reventaba con `ModuleNotFoundError: tomllib` (Python < 3.11 fuera del contenedor) — corregido con import opcional + fallback a un regex acotado al `[project]` de `pyproject.toml`; y `scripts/` no está dentro de la imagen de la API (`docker exec … scripts/…` no lo encuentra), así que el script se corre desde el checkout del host, no por `docker exec`.

Cliente de validación `taller-prueba` retirado: Subscription cancelada en el dashboard de Stripe y clave local desactivada (`UPDATE api_keys SET active=0`, 1 fila). `CHECKLIST-operativa.md` y `MASTERPLAN.md` marcan despliegue y guardarraíl como verificados en real. Queda pendiente solo la verificación del cobro de overage de punta a punta (requiere superar una cuota real).
