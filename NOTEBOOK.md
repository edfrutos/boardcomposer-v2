...
Nota de seguridad, no resuelta: en el proceso de diagnosticar y reconstruir ambos contenedores, el usuario compartió en el chat, en texto plano, `BOARDCOMPOSER_API_KEY`, la contraseña de `auth_basic` de nginx (`bc-studio`), `VNC_PASSWORD` (con el mismo valor que `BOARDCOMPOSER_API_KEY` — credencial reutilizada entre dos servicios) y una clave real de `ANTHROPIC_API_KEY`. Ninguna se ha confirmado rotada todavía — pendiente, anotado en `MASTERPLAN.md`.
